...
## Modification to Ctune token cli commands
  ...
  2. add Platfom-> SE Manager component. indeed on series 2 USERDATA flash page used to store tokens is managed by the Secure element, therefore direct memory access is not allowed.

  3. modify your app.c file with this content. It adds several cli command. 
  ...
  if you save additionnal tokens or any data in the User data page, you will need to save before erasing and writing them back, therefore modify the function to do so.
    ...
    ******************************************************************************/
    ...
    uint32_t ctune_offset = 0x100>>2; //CTUNE base
    uint32_t value;
    ...
    void SetCtuneToken (sl_cli_command_arg_t *args)
    {
      ...
      if (USERDATA[ctune_offset] == 0xffffffff)
        {
        status = sl_se_write_user_data(&cmd_ctx, ctune_offset<<2, &value, 4);
        if (status == 0)
    ...
    void EraseCtuneTokenPage (sl_cli_command_arg_t *args)
    {
      ...
      status = sl_se_erase_user_data(&cmd_ctx);
      if (status == 0)
